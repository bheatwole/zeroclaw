...
impl ComponentTool {
    ...
    pub fn from_bytes(engine: Arc<ComponentEngine>, bytes: &[u8]) -> anyhow::Result<Self> {
        ...
        let name = exports.call_name(&mut store).map_err(PluginError::from)?;
...
impl Tool for ComponentTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        tokio::task::spawn_blocking(move || {
            logging::wrap_plugin_call(&plugin_name, &plugin_version, "execute", || {
                let mut store = wasmtime::Store::new(engine.engine(), PluginLoggingHost);
                let bindings = pre.instantiate(&mut store).map_err(PluginError::from)?;
                let exports = bindings.zeroclaw_plugin_tool();
