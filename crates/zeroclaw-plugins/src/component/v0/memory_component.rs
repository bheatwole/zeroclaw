...
impl ComponentMemory {
    ...
    pub fn from_bytes(
        ...
        let mut store = wasmtime::Store::new(engine.engine(), PluginLoggingHost);
...
// ── Helper macro for spawn_blocking calls ─────────────────────────────────────
...
macro_rules! call_plugin {
    ($self:expr, $op:literal, $block:expr) => {{
        ...
        tokio::task::spawn_blocking(move || {
            super::logging::wrap_plugin_call(&plugin_name, &plugin_version, $op, || {
...
impl zeroclaw_api::memory_traits::Memory for ComponentMemory {
    ...
    async fn store(
        ...
        call_plugin!(self, "store", move |store, bindings: &mut MemoryPlugin| {
            bindings
                .zeroclaw_plugin_memory()
    ...
    async fn recall(
        ...
        call_plugin!(self, "recall", move |store, bindings: &mut MemoryPlugin| {
            bindings
                .zeroclaw_plugin_memory()
                .call_recall(
                    store,
    ...
    async fn get(&self, key: &str) -> anyhow::Result<Option<MemoryEntry>> {
        let key = key.to_string();
        call_plugin!(self, "get", move |store, bindings: &mut MemoryPlugin| {
            bindings
                .zeroclaw_plugin_memory()
                .call_get(store, &key)
                .map_err(anyhow::Error::msg)?
    ...
    async fn list(
        ...
        let session_id = session_id.map(str::to_string);
        call_plugin!(self, "list", move |store, bindings: &mut MemoryPlugin| {
            bindings
                .zeroclaw_plugin_memory()
    ...
    async fn forget(&self, key: &str) -> anyhow::Result<bool> {
        let key = key.to_string();
        call_plugin!(self, "forget", move |store, bindings: &mut MemoryPlugin| {
            bindings
                .zeroclaw_plugin_memory()
    ...
    async fn forget_for_agent(&self, key: &str, agent_id: &str) -> anyhow::Result<bool> {
        ...
        call_plugin!(
            self,
            "forget_for_agent",
            move |store, bindings: &mut MemoryPlugin| {
                bindings
                    .zeroclaw_plugin_memory()
    ...
    async fn count(&self) -> anyhow::Result<usize> {
        call_plugin!(self, "count", move |store, bindings: &mut MemoryPlugin| {
            bindings
                .zeroclaw_plugin_memory()
                .call_count(store)
                .map_err(anyhow::Error::msg)?
